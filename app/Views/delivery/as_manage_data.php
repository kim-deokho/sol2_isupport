...
<?
    if($totCnt>0) {
        foreach($rows as $row) {
            ...
            echo '  <td><button type="button" class="small bt_sblue" onclick="as_proc(\''.$row['ma_pid'].'\');">보기</button></td>';
            ...
            echo '  <td><button type="button" class="small bt_black" onclick="disposal_parts();">등록</button></td>';
            echo '  <td>'.number_format($row['aa_total_price']).'</td>';
            echo '  <td><label class="chkWrap"><input type="checkbox" name="payment_yn" value="Y"  onclick="IsPayment('.$row['aa_pid'].')" '.($row['aa_payment_yn']=='Y'?'checked':'').' /><i></i></label></td>';
        }
    }
